...
@DisplayName("Репозиторий на основе SpringData для работы с комментариями ")
@DataJpaTest
class CommentRepositoryTest {
    ...
    void shouldReturnCorrectCommentsByBookId() {
        ...
        val comments = commentRepository.findByBook_Id(FIRST_BOOK_ID);
    ...
    void shouldUpdateCommentTextByBookId() {
        val editFirstComment = commentRepository.findByBook_Id(FIRST_BOOK_ID).stream().findFirst().get();
        String oldText = editFirstComment.getText();
        em.detach(editFirstComment);
        editFirstComment.setText(NEW_COMMENT_TEXT);
        commentRepository.save(editFirstComment);
